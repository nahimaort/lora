...
const uint8_t localAddress = 0x71;     // Dirección de este dispositivo
uint8_t destination = 0x70;            // Dirección de destino, 0xFF es la dirección de broadcast
...
void loop() 
{
  ...
  static uint32_t contador_dummies = 0;
  if(!transmitting && ((millis() - lastSendTime_ms) > txInterval_ms) && flag_envio){
    if(flag_cambio){
      aplicar_cambios();
      flag_cambio = false;
    }
    delay(2000);
    if(contador_dummies < 5){
      enviar_dummy();
      contador_dummies++;
    }else{
      enviar_ack();
...
void onReceive(int packetSize) 
{
  ...
  if(remoteNodeConf.bandwidth_index != thisNodeConf.bandwidth_index||
...
void enviar_ack(){
  ...
  Serial.println("empiezo a enviar el ACK...");
...
void enviar_dummy(){
  ...
  uint8_t payload_dummy[50];
  uint8_t payloadLength_dummy = 4;
  ...
  sendMessage(payload_dummy, payloadLength_dummy, 0);
  Serial.print("SENDING DUMMY PACKET: ");
  printBinaryPayload(payload_dummy, payloadLength_dummy);
  Serial.println();
